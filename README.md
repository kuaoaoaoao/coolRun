...
coolRun 把实时金价和 Mac 状态放进菜单栏。

它是一款轻量的 macOS 菜单栏工具，可以直接显示浙商银行积存金价格。点击菜单栏图标后，可以查看 CPU、内存、储存、电池、网络、温度、风扇和系统运行时间。

一个小细节：菜单栏里的金币图标会根据 CPU 占用加速旋转。电脑越忙，金币转得越快。
...
- 你想知道 Mac 当前是不是高负载、内存是否紧张、风扇为什么响。
...
- CPU：核心数、实时占用、动态占用条、健康状态颜色编码。
- 内存：已用内存、总内存、内存压力、健康状态颜色编码。
- 储存：已用空间、可用空间、使用进度。
- 电池：电量、充电状态、低电量模式。
...
- 毛玻璃背景，接近 macOS 控制中心风格。
- 可折叠面板，默认保持紧凑。
- CPU、内存、网速趋势图，保留最近 60 个采样点。
- 点击任意指标可复制到剪贴板。
- 跟随系统浅色和深色模式。
...
<table>
  <tr>
    ...
    <td align="center">
      <img src="docs/images/dark-main.png" width="260" alt="深色模式">
      <br>
      <sub>深色模式</sub>
    </td>
  </tr>
  <tr>
    <td align="center">
      <img src="docs/images/light-montring.png" width="260" alt="浅色模式">
      <br>
      <sub>浅色模式</sub>
    </td>
    <td align="center">
      <img src="docs/images/settings.png" width="420" alt="设置页面">
      <br>
      <sub>设置页面</sub>
    </td>
  </tr>
