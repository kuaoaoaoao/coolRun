...
- 系统监控
  ...
  - 风扇：风扇转速（RPM）、转速百分比、颜色编码。

- 界面功能
  - 毛玻璃效果：现代半透明背景，类似 macOS 控制中心风格。
  - 可折叠面板：点击任意区域展开/收起详细内容，默认收起状态。
  - 趋势图表：CPU、内存、网速的历史趋势图（最近 60 个采样点）。
  - 点击复制：点击任意指标可复制到剪贴板。
  - 颜色编码：CPU 和内存使用率用绿/黄/红颜色表示健康状态。
  - 紧凑设计：更小的图标、更紧凑的间距、更高的信息密度。
